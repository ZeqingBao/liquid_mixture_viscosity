...
def pchip_interpolation(nist_knovel_all, test_mols):
    ...
    for single_entry in one_off_data:
        single_dict = {
            ...
            "MolFrac_1": float(single_entry[3][0]) if isinstance(single_entry[3], np.ndarray) else single_entry[3],
            "logV": float(single_entry[4][0]) if isinstance(single_entry[4], np.ndarray) else single_entry[4],
            "T": single_entry[5],
            "Avg_Series_Unc": single_entry[6],
        }
        single_df = pd.DataFrame([single_dict])
        input_data = pd.concat((input_data, single_df), ignore_index=True)
